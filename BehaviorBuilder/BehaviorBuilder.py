...
class BehaviorFile:
    ...
    def add_state(self, name, animation_path="placeholder", looping=False, gamebryoanim=False, enterNotifyEvents="null",exitNotifyEvents="null"):
        ...
        if not gamebryoanim and animation_path=="placeholder":
            log.warning("If adding a .hkx animation state, the animation_path argument must be passed: 'animations\\<your anim name.hkx>")
            return
    ...
    def add_wildcard(self, stateStr, event):
        ...
        wildcardtransitionsOBJ.add_transition(eventIdIdx=eventIndex, toStateIdIdx=stateInfo.stateIdx, transitionStr=self.blend_effect.tag, wildcard=True)
...
class hkbStateMachineStateInfo:
    ...
    def __init__(self, BehaviorFileObj, generator, transitions,  name, state_ID, enterNotifyEvents="null",exitNotifyEvents="null"):
        ...
        assert (exitNotifyEvents.__class__.__name__ == "hkbStateMachineEventPropertyArray" or (isinstance(exitNotifyEvents,str) and exitNotifyEvents=="null"))
        assert (enterNotifyEvents.__class__.__name__ == "hkbStateMachineEventPropertyArray" or (isinstance(enterNotifyEvents,str) and enterNotifyEvents=="null"))
        assert isinstance(name,str)
...
class hkbStateMachine:
    ...
    def __init__(self,BehaviorFileObj, name="defaultNameBehavior", startStateIdIdx=0):
        ...
        self.eventToSendWhenStateOrTransitionChangesObject=ET.SubElement(self.eventToSendWhenStateOrTransitionChanges,'hkobject')
        #idparam
